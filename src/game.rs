...
impl Game {
    ...
    pub fn update(
        ...
        if self.state != GameStates::Paused {
            ...
            // Edge collisions
            if ball_collides_with_top(&self.ball) || ball_collides_with_bottom(&self.ball) {
                self.ball.flip_y();
            }

            if ball_collides_with_left(&self.ball) || ball_collides_with_right(&self.ball) {
                self.ball.flip_x();
            }

            //  Paddle one collision
            if ball_collides_with_paddle_one(&self.ball, &self.paddle_one) {
                handle_ball_paddle_collision(&mut self.ball, &self.paddle_one);
            }

            if ball_collides_with_paddle_two(&self.ball, &self.paddle_two) {
                handle_ball_paddle_collision(&mut self.ball, &self.paddle_two);
            }
        }
    }
}
